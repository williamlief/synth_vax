...
results<- vaccine_out %>% grab_signficance() %>% mutate(scaling_factor=i) %>% bind_cols(last_period_diff)
#print(i)
}-> power_analysis
print(i)  
...
```{r}
power_results %>% write_csv(here("output/bayes_factor_6_23.csv"))
power_results <- read_csv(here("output/bayes_factor_6_23.csv"))
...
```{r}
power_results %>% filter(.id=="OH") %>% group_by(scaling_factor) %>% summarise(mean(last_period_diff>-1.3))
