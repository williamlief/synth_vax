...
```{r}
power_results %>% write_csv(here("output/bayes_factor_8_29.csv"))
power_results <- read_csv(here("output/bayes_factor_8_29.csv"))
...
```{r}
power_results %>% filter(.id=="OH") %>% group_by(scaling_factor) %>% summarise(mean(last_period_diff<0))
